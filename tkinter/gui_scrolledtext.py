...
def click_me():
    action.configure(text = "Clicked Me! "+ name.get()+" "+number_chosen.get()+" "+str(chVarUn.get())+" "+str(radVar.get()))
...
for col in range(3):
    curRad = tk.Radiobutton(window, text=color[col], variable=radVar, value=col, command=radCall)
...
scroll = scrolledtext.ScrolledText(window,  width = scroll_w, height = scroll_h, wrap=tk.WORD)
